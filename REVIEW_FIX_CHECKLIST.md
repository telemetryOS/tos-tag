...
eval, gosec, govulncheck, and focused real-Mongo tests passing; commit still
pending): `NEW-1`, `F2`, `F3`, `F19`, `F5`, `F6`, `F7`,
...
- [ ] `NEW-1` Recover actionable top-level direct messages missed while offline.
- [ ] `F2` Make bounded catch-up progress without wedging later channels or skipping gaps.
- [ ] `F3` Persist all supported Mongo job transition mutations, including resolved routing.
- [ ] `F19` Recover offline replies in existing Tag threads whose roots predate the watermark.
- [ ] `F6` Reconcile a succeeded job whose final delivery enqueue failed.
- [ ] `F7` Reserve observation output atomically before enqueueing user-visible work.
- [ ] `F41` Make admission reservation completion and active-count release recoverable.
- [ ] `F5` Persist and verify `/tag-mode` audit receipts.

## Batch 2 — worker lifecycle and output safety

- [ ] `F8` Treat transient job-read failures as retryable rather than revocation.
- [ ] `F9` Clean up sessions on every `runHarness` early return.
- [ ] `F24` Remove bounded-channel sends from the worker-session mutex critical section.
- [ ] `F26` Terminate provisioned workers on prompt validation failures.
- [ ] `F18` Validate unlabeled Slack angle-bracket URLs against the safe-scheme policy.
- [ ] `F1` Preserve legitimate fenced code in plain-text model output.
- [ ] `F16` Preserve literal identifier punctuation in rich-text table cells.
- [ ] `F25` Report cumulative per-turn token usage.
- [ ] `F38` Make audit content commitments durable and verifiable across restarts.
- [ ] `F40` Require or resolve the Slack bot user ID in Socket Mode.

## Batch 3 — production query behavior and operator correctness

- [ ] `F30` Add indexes matching job claim, recovery, heartbeat, and public-ID queries.
- [ ] `F32` Stop loading every job on each reconciliation tick.
- [ ] `F20` Bound bot-membership discovery without failing on large public-channel inventories.
- [ ] `F31` Add indexes matching observation claim and public-ID mutation queries.
- [ ] `F15` Bound and filter decision-list queries.
- [ ] `F23` Remove unauthenticated status-path full-collection scans.
- [ ] `F21` Make learned-note listing use the persisted schema.
- [ ] `F22` Return recent work newest-first with bounded tenant lists.
- [ ] `F4` Allow forgotten memory scopes to be relearned under the tombstone contract.
- [ ] `F35` Prevent generated memory from overwriting concurrent operator corrections.
- [ ] `F36` Remove unnecessary projection round trips for ordinary new messages.
- [ ] `F34` Align memory and Mongo observation projection/claim semantics.
- [ ] `F39` Consolidate background-work scope authorization without widening authority.
- [ ] `F13` Generalize external public-source detection beyond a single vendor.
- [ ] `F10` Match incident terms on token boundaries.

## Batch 4 — confirmed lower-severity maintenance

- [ ] `F11` Stop full job scans on every reconciliation poll.
- [ ] `F12` Keep policy-correction reactions inside the configured allowlist.
- [ ] `F14` Consolidate social-reply selection behavior.
- [ ] `F17` Align exhausted-delivery handling in Mongo and memory queues.
- [ ] `F27` Preserve the graceful worker-termination window.
- [ ] `F29` Align bounded marketplace file reads with the tools implementation.
- [ ] `F33` Remove or integrate production-dead job operations.
- [ ] `F37` Remove or integrate the production-dead conversational-search package.
- [ ] `F42` Make the memory organization store fail closed like Mongo.
- [ ] `F43` Preserve an active directive when replacement activation fails.
- [ ] `F44` Prevent duplicate keyless audit receipts during CAS retries.
- [ ] `F45` Consolidate duplicated routine and trigger behavior.
- [ ] `F46` Scope in-memory routine IDs by organization.
- [ ] `F47` Batch expired derivation deletion.
- [ ] `F48` Remove the misplaced delivery `WriterActive` field.
- [ ] `F49` Share response-profile defaults with live evals.
- [ ] `F50` Share fixture-contract validation between deterministic and live evals.
- [ ] `F52` Make activity text truncation UTF-8 safe.
...
- [ ] `F104` Verify activity truncation UTF-8 behavior.
