...
func TestOpenAIClassifierUsesDirectStructuredResponsesAPI(t *testing.T) {
	...
	if !classifierPayload.DirectMention || classifierPayload.DirectMessage || classifierPayload.ActiveThread {
...
func TestAmbientPolicyCorrectionsSurfaceAlignmentAndEnforceAdmittedPlacement(t *testing.T) {
	...
	writeSilent := withSourceWritePolicyCorrections(types.ClassificationDecision{Outcome: types.OutcomeStartBackgroundJob, Confidence: .95, ReasonCodes: []string{"implementation"}, SourceWriteRequested: true, ProductRetrievalRequired: true}, Target{Envelope: types.SlackEnvelope{Text: "Implement this fix in Gateway-Service."}})
	if writeSilent.Outcome != types.OutcomeSilent || writeSilent.DirectReply != "" || writeSilent.Reaction != "" || !writeSilent.SourceWriteRequested || writeSilent.ProductRetrievalRequired || writeSilent.RequiresFullAgent || writeSilent.AgentModelStrength != "none" || !slices.Contains(writeSilent.ReasonCodes, "policy.source_write_silent") {
		t.Fatalf("source write was not silent = %#v", writeSilent)
	}
	wikiEdit := withSourceWritePolicyCorrections(types.ClassificationDecision{Outcome: types.OutcomeReplyInThread, Confidence: .95, ReasonCodes: []string{"artifact_update"}, SourceWriteRequested: true, DirectReply: "unexpected source-write response", RequiresFullAgent: true, AgentModelProfile: "chatgpt-luna-low", AgentModelStrength: "light", AgentReasoningEffort: "low"}, Target{ActiveThread: true, Envelope: types.SlackEnvelope{Text: "Add a validation section to the architecture reference you just published."}})
	...
	wikiEditWithSourceText := withSourceWritePolicyCorrections(types.ClassificationDecision{Outcome: types.OutcomeReplyInThread, Confidence: .95, SourceWriteRequested: true, DirectReply: "unexpected source-write response"}, Target{ActiveThread: true, Envelope: types.SlackEnvelope{Text: "Add a short validation section to the architecture reference covering privacy refusal and source-write suppression."}})
	...
	if mixedWikiAndSourceWrite.Outcome != types.OutcomeSilent || !mixedWikiAndSourceWrite.SourceWriteRequested || mixedWikiAndSourceWrite.DirectReply != "" {
	...
	doneToday := withSourceWritePolicyCorrections(types.ClassificationDecision{Outcome: types.OutcomeSilent, Confidence: .99, ReasonCodes: []string{"ambient_declarative_update", "no_action_requested"}, SourceWriteRequested: true, DirectReply: "unexpected source-write response"}, Target{Mode: types.ModeAssist, Envelope: types.SlackEnvelope{Text: doneTodayText}})
	...
	conceptPost := "TelemetryCode — concept summary (still in ideation). We are considering an AI-delivered application build and managed-service line. Nothing here is approved or committed. Fixed prices and managed adoption are part of the proposal, with a runtime verification gate rather than code review alone. Feedback welcome; this is ideation, not a launch plan."
	if isObviousSourceWriteRequest(conceptPost) {
		t.Fatal("TelemetryCode concept post was mistaken for a source mutation request")
	}
	...
	providerWriteFlag := withReadOnlyCodeAnalysisPolicyCorrections(types.ClassificationDecision{Outcome: types.OutcomeReplyInChannel, Confidence: .99, DirectReply: "unexpected source-write response", SourceWriteRequested: true, Reaction: "speech_balloon", AgentModelStrength: "none"}, Target{Envelope: types.SlackEnvelope{Text: "Review the Gateway authentication code and explain how token validation works."}}, profiles)
	...
	continuityReview := withReadOnlyCodeAnalysisPolicyCorrections(types.ClassificationDecision{Outcome: types.OutcomeReplyInChannel, Confidence: .99, DirectReply: "unexpected source-write response", SourceWriteRequested: true, Reaction: "speech_balloon", AgentModelStrength: "none"}, Target{Envelope: types.SlackEnvelope{Text: "Review tos-tag's conversation-continuity admission gate for obvious false positives or false negatives."}}, profiles)
