...
func (*imageHarness) Health(context.Context) error { return nil }
func (*imageHarness) CreateSession(context.Context, string) (harness.Session, error) {
	return harness.Session{ID: "image-session", CreatedAt: time.Now().UTC()}, nil
}
func (h *imageHarness) Prompt(_ context.Context, _ string, prompt harness.Prompt) error {
	h.prompt = prompt
	return nil
}
func (*imageHarness) Events(context.Context, string) (<-chan harness.Event, <-chan error) {
	png := []byte{0x89, 'P', 'N', 'G', '\r', '\n', 0x1a, '\n'}
	events := make(chan harness.Event, 3)
	errs := make(chan error)
	events <- harness.Event{Type: "file.produced", Data: map[string]any{"file": types.SlackFileUpload{Name: "generated.png", MediaType: "image/png", Data: png}}}
	events <- harness.Event{Type: "message.delta", Data: map[string]any{"text": `{"segments":[{"kind":"mrkdwn_text","text":"generated"}]}`}}
	close(events)
	close(errs)
	return events, errs
}
func (*imageHarness) Permission(context.Context, string, harness.PermissionDecision) error {
	return nil
}
...
func (m staticImageMedia) DownloadImages(context.Context, string, []types.SlackImageRef) ([]types.SlackImageData, error) {
	return []types.SlackImageData{{Name: "input.png", MediaType: "image/png", Data: append([]byte(nil), m.data...)}}, nil
}
...
func TestFullAgentCarriesSlackImageInputAndGeneratedFileOutsideModelJSON(t *testing.T) {
	cfg := config.DefaultConfiguration
	cfg.Jobs.Lease = time.Minute
	png := []byte{0x89, 'P', 'N', 'G', '\r', '\n', 0x1a, '\n'}
	h := &imageHarness{}
	p := &Pipeline{deps: Dependencies{Config: &cfg, Harness: h, Media: staticImageMedia{data: png}}}
	result, err := p.runHarness(context.Background(), jobs.Job{
		ID: "job-image", OrganizationID: "org-test", WorkspaceID: "team-test", ChannelID: "tos-tag",
		Input: `{"request":"describe this image","image_attachment_count":1}`, Images: []types.SlackImageRef{{FileID: "F1", MediaType: "image/png", Size: len(png)}},
		ResolvedModel: types.ResolvedModel{ProviderID: "openai", ModelID: "gpt-5.6-luna", Variant: "medium"},
	})
	if err != nil {
		t.Fatal(err)
	}
	if len(h.prompt.Images) != 1 || string(h.prompt.Images[0].Data) != string(png) {
		t.Fatalf("prompt images=%#v", h.prompt.Images)
	}
	if len(result.Files) != 1 || result.Files[0].MediaType != "image/png" || string(result.Files[0].Data) != string(png) {
		t.Fatalf("generated files=%#v", result.Files)
	}
	encoded, _ := json.Marshal(result)
	if strings.Contains(string(encoded), "generated.png") || strings.Contains(string(encoded), "iVBOR") {
		t.Fatalf("generated bytes entered model JSON: %s", encoded)
	}
}

func TestExplicitImageGenerationIntentIsBounded(t *testing.T) {
	for _, value := range []string{"generate an image of a fox", "make me a 16:9 graphic", "an illustration, please create one"} {
		if !explicitImageGenerationRequested(value) {
			t.Fatalf("explicit request not recognized: %q", value)
		}
	}
	for _, value := range []string{"describe this image", "the build produced an image", "we should discuss graphic design"} {
		if explicitImageGenerationRequested(value) {
