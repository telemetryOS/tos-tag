...
func TestLoadSlackContextSyncEnvironment(t *testing.T) {
	...
	t.Setenv("TAG__SLACK__AUTOMATION_DEFAULT_TIMEZONE", "America/Vancouver")
	...
	if cfg.Slack.AutomationDefaultTimezone != "America/Vancouver" {
		t.Fatalf("Slack automation default timezone did not map: %q", cfg.Slack.AutomationDefaultTimezone)
	}
...
func TestValidateSlackAutomationDefaultTimezone(t *testing.T) {
	cfg := DefaultConfiguration
	cfg.Slack.AutomationDefaultTimezone = "not/a-zone"
	if err := Validate(&cfg); err == nil || !strings.Contains(err.Error(), "valid IANA timezone") {
		t.Fatalf("invalid automation timezone error = %v", err)
	}
}
