...
- Treat Slack images as control-plane media: persist bounded file IDs only,
  fetch and validate them after admission, and pass local disposable image
  files to App Server. Curds generation must use the reviewed `media.curds`
  operation and the `curds` skill; workers never receive Slack or OpenAI keys,
  generated bytes, an output path, or destination choice.
...
- Keep automation identity channel-scoped across indexes, repositories,
  ...
  `/tag-automations` may list, create, edit, and delete only in the invoking
  channel; existing records may never be moved to another channel by an edit.
...
- `media.curds` (`generate`, no approval for explicit image requests): fixed
  OpenAI `gpt-image-2`, one WebP artifact, keystore-only API key, and durable
  current-destination Slack file publication;
...
Linear, Wiki, SigNoz, DLA, Curds OpenAI, and optional Site Analytics and Attio variables from the current shell or
