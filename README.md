...
Slack PNG, JPEG, and WebP attachments are retained only as bounded file
identities. After admission, the bot-authenticated control plane downloads and
validates up to four images, writes them into the disposable worker, and sends
them to App Server as `localImage` inputs. Explicit image-generation requests
use reviewed `media.curds/generate`; Curds receives `OPENAI_API_KEY` only through
the encrypted tool binding, and generated bytes are durably published to the
current authorized Slack channel or thread.
...
| Curds | `9c746c5532ab…` (`0.1.0`) | Reviewed OpenAI `gpt-image-2` image generation |
...
| tag-agent-skills | plugin `base` `1.7.0`; current configured checkout | Complete 33-skill tos-tag behavioral package |
...
The currently injected behavioral skill inventory is `base` (33): `attio`, `bug`,
`code-change-intake`, `codebase-read`, `digitalocean`, `feature`, `humanizer`,
`curds`, `linear-issue-manager`, `marketing-account-journey`,
...
| `media.curds` | `generate` | Never for explicit generation requests | One OpenAI `gpt-image-2` WebP artifact, published only to the current Slack destination | Enabled when `OPENAI_API_KEY` and Curds are available |
...
without per-action approval. `media.curds/generate` is limited by the validated
`curds` skill, fixed provider/model/output, bounded arguments, and current
destination. Recoverable Wiki page soft-delete always requires
...
the Curds OpenAI key, optional Attio access token, Stripe API key, DigitalOcean API key, and
...
- Keep Slack file URLs and Slack/OpenAI credentials out of workers; authenticated
  image download and generated-file publication are control-plane-owned.
